...
int main()
{
	...
	while (window.isOpen())
	{
        ...
        std::vector<std::tuple<float, Pvector>> asteroidSizeAndPos = asteroidField.GetSizeAndPosition();
    ...
    endPlayerFactoryCol:
        ...
        {
            //+ Player
            if (asteroidBounds[i].intersects(playerBound))
            {
                //Damage Player
                goto endAsteroidCollision;
            }

        }
    endAsteroidCollision:
